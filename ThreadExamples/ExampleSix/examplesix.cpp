/*
std::try_lock
std::try_lock() tries to lock all the lockable objecs passed in it oneby one in given order
SYNTAX std::try_lock(m1,m2,m3...)

on success this function returns -1 otherwise it will return 0-bawsed mutex index number which it 
could not lock

if it fails to lock any of the mutex then it will release all the mutex it locked before.

if a call to try_lock results in an exception, unlock is called for any locked objects before
rethrowing.
*/
