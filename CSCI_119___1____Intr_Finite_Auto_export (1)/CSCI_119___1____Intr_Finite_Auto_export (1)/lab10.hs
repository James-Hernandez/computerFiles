...
--LABWORK
complement :: Ord a => FSM a -> FSM a
complement (q1,s,f,d) = (qs, ss, ff, dd) where
    qs = q1
    ss = s
    ff = [q | q<-q1, notElem q f]
    dd = d
...
reverseFSM (qs,s1,f1,d1) = (q,s,f,d) where
    q = qs
    s = f1
    f = [s1] --all the start states of previous machine
    d q a = [q1 | q1 <- qs, q == (d1 q1 a)]
...
{-
test_bin =checkFSM$intify$ binopFSM (&&) testMachine testMachine1 
--TRUE
test_bin1 =checkFSM$intify$ binopFSM (||) testMachine testMachine1 
--TRUE
test_min =checkFSM$intify$ minimize testMachine
--UNDEFINED D
test_compl = checkFSM$intify$ complement testMachine 
--TRUE
test_accC = accept1 (intify$ complement testMachine) ['a','b'] 
--FALSE
test_accC1 = accept1 (intify$ complement testMachine) ['b','a'] 
--FALSE
test_machine = accept1 (intify$ binopFSM (&&) testMachine testMachine1) ['a'] 
--TRUE
test_machine01 = accept1 (intify$ binopFSM (&&) testMachine testMachine1) ['a','b'] 
--FALSE

test_machine1 = accept1 (intify$ binopFSM (||) testMachine testMachine1) ['a','b'] 
--TRUE
-}
--End LABWORK
