...
*Main> proper (Let' 'a')
True
*Main> proper (Union' [Let' 'a', Zero])   
True
*Main> proper (Cat' [Let' 'a',One])
True
*Main> proper (Star' Zero)
False
-}
...
solve ((l11:l1J) : rows) (l1':lI') = simp x1 : xI where
  ...
  -- compute x1 from xI via (5) --help from jack
  x1 =Cat' [Star' l11, Union' (zipWith (\i j -> Cat' [i, j]) l1J xI ++ [l1'])]
...
empty:: FSM Int
empty = (qs,s,fs,d) where
  qs = [0]
  s = 0
  fs = []
  d 0 _ = 0

convTest1 = conv end_ab -- will return RegExp'
--Cat' [Star' (Let' 'b'),Let' 'a',Star' (Let' 'a'),Let' 'b',Star' (Union' [Cat' [Let' 'a',Star' (Let' 'a'),Let' 'b'],Cat' [Let' 'b',Star' (Let' 'b'),Let' 'a',Star' (Let' 'a'),Let' 'b']])]
convTest2 = conv empty
--Zero
fromRE'Test = fromRE' convTest1
--Cat (Star (Let 'b')) (Cat (Let 'a') (Cat (Star (Let 'a')) (Cat (Let 'b') (Star (Union (Cat (Let 'a') (Cat (Star (Let 'a')) (Let 'b'))) (Cat (Let 'b') (Cat (Star (Let 'b')) (Cat (Let 'a') (Cat (Star (Let 'a')) (Let 'b'))))))))))
fromRE'Test2 = fromRE' convTest2
--Empty
listParam2 = map toRE' ["a*","ab.","aa+"]
--[Star' (Let' 'a'),Cat' [Let' 'a',Let' 'b'],Union' [Let' 'a',Let' 'a']]
listParma1 = [map toRE'["a*","ab.","aa+"]]
--[[Star' (Let' 'a'),Cat' [Let' 'a',Let' 'b'],Union' [Let' 'a',Let' 'a']]]
solveTest1 = solve listParma1 listParam2
--[Cat' [Star' (Let' 'a'),Star' (Let' 'a')]]
listParam2' = map toRE' ["ab.","b*","bb+"]
--[Cat' [Let' 'a',Let' 'b'],Star' (Let' 'b'),Union' [Let' 'b',Let' 'b']]
solveTest2 = solve listParma1 listParam2'
--[Cat' [Star' (Let' 'a'),Let' 'a',Let' 'b']]
toSPLETest1 = toSPLE end_ab
--([[Let' 'b',Let' 'a',Zero],[Zero,Let' 'a',Let' 'b'],[Let' 'b',Let' 'a',Zero]],[Zero,Zero,One])
toSPLETest2 = toSPLE oddbs
--([[Let' 'a',Let' 'b'],[Let' 'b',Let' 'a']],[Zero,One])
properTest1 = proper (Let' 'a')
--True
properTest2 = proper (Union' [Zero, One])
--False
